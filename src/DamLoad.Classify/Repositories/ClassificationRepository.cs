...
namespace DamLoad.Classify.Repositories
{
    public class ClassificationRepository : IClassificationRepository
    {
        ...
        public async Task<List<ClassificationEntity>> GetByResourceIdAsync(string resourceId)
        {
            ...
            string sql = "SELECT * FROM classifications WHERE resource_id = @ResourceId ORDER BY sort_order ASC";
        ...
        {
            using var db = GetConnection();
            string sql = "DELETE FROM classifications WHERE resource_id = @ResourceId AND classifier_id = @ClassifierId";
